...
namespace CampusEats.Api.Tests.Validators.Payments
{
    public class PaymentConfirmationValidatorTests : IDisposable
    {
        ...
        private PaymentConfirmationValidator CreateSUT() => new();
        ...
        {
            // Arrange
            var request = new PaymentConfirmationRequest(Guid.NewGuid(), PaymentStatus.Successful);
        ...
        {
            // Arrange
            var request = new PaymentConfirmationRequest(Guid.Empty, PaymentStatus.Successful);
        ...
        {
            // Arrange
            var request = new PaymentConfirmationRequest(Guid.NewGuid(), PaymentStatus.Initiated);
            ...
            result.Errors.Should().Contain(e => e.PropertyName == "NewStatus" && e.ErrorMessage.Contains("Initiated"));
        }
